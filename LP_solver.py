...
def auxiliary_method(obj, obj_vars, lhs, lhs_vars, rhs):
	...
	# loop through the first col of lhs_vars to find rows that looks like x_n = b_n + a_11*...
...
def largest_coeff_pivot(obj, obj_vars, obj_val, lhs, lhs_vars, rhs):
	...
	while not is_optimal(obj, rhs):
		...
		for b in rhs:
			if b < 0:
				obj, obj_vars, lhs, lhs_vars, rhs = auxiliary_method(obj, obj_vars, lhs, lhs_vars, rhs)
				'''
